...
function aktifkanSuara() {
  ...
  if (!btn) {
    const playBtn = document.querySelector('.btn-mulai, .btn-mulai-large, .hero__btn-play');
...
function beritaSebelumnya() {
  ...
  if (section && index !== null && typeof newsData !== 'undefined') {
    ...
    else if (section === 'beritaInklusif_grid') arr = newsData.beritaInklusif.grid;
    else if (section === 'pilihanRedaksi') arr = newsData.pilihanRedaksi;
    if (arr && parseInt(index) - 1 >= 0) {
      window.location.href = '/src/page/structure/detailBerita.html?section=' + section + '&index=' + (parseInt(index) - 1);
    }
  }
}
